...
module "fee_calculator_api__external__oauth" {
  source              = "git::https://github.com/pagopa/terraform-azurerm-v3.git//api_management_api?ref=v5.1.0"
  name                = "${local.project}-fee-calculator-external-oauth"
  ...
  path = "ext/aut/mil-fee-calculator"

  display_name          = "fee calculator - external - oauth"
...
resource "azurerm_api_management_api_diagnostic" "fee_calculator_api__external__oauth" {
  identifier               = "applicationinsights"
  resource_group_name      = module.apim.resource_group_name
  api_management_name      = module.apim.name
  api_name                 = module.fee_calculator_api__external__oauth.name
