...
module "payment_notice_api__external__api_key" {
  source              = "git::https://github.com/pagopa/terraform-azurerm-v3.git//api_management_api?ref=v5.1.0"
  name                = "${local.project}-payment-notice-external-apikey"
  ...
  path = "ext/sub/mil-payment-notice"

  display_name          = "payment notice - external - api key"
...
resource "azurerm_api_management_api_diagnostic" "payment_notice_api__external__api_key" {
  identifier               = "applicationinsights"
  resource_group_name      = module.apim.resource_group_name
  api_management_name      = module.apim.name
  api_name                 = module.payment_notice_api__external__api_key.name
