...
#
locals {
  auth_kv_ca_name = "${local.project}-auth-kv-ca"
}

resource "azurerm_resource_group_template_deployment" "mil_auth_kv" {
  name                = local.auth_kv_ca_name
  resource_group_name = azurerm_resource_group.app.name
  deployment_mode     = "Incremental"
  tags                = var.tags

  template_content = templatefile("templates/mil-auth-kv.json",
    {
...
locals {
  mil_auth_kv_ingress_fqdn = jsondecode(azurerm_resource_group_template_deployment.mil_auth_kv.output_content).ingress_fqdn.value
}
