...
module "acquirer_conf_api__internal" {
  ...
  path = "int/mil-acquirer-conf"

  display_name          = "acquirer conf - internal"
  content_format        = "openapi-link"
  content_value         = "https://raw.githubusercontent.com/pagopa/mil-apis/main/openapi-mono/acquirer-conf.yaml"
  product_ids           = [module.mil_product__internal.product_id]
...
resource "azurerm_api_management_api_diagnostic" "acquirer_conf_api__internal" {
  identifier               = "applicationinsights"
  resource_group_name      = module.apim.resource_group_name
  api_management_name      = module.apim.name
  api_name                 = module.acquirer_conf_api__internal.name
