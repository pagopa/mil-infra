...
module "fee_calculator_api__external__api_key" {
  source              = "git::https://github.com/pagopa/terraform-azurerm-v3.git//api_management_api?ref=v5.1.0"
  name                = "${local.project}-fee-calculator-external-apikey"
  ...
  path = "ext/sub/mil-fee-calculator"

  display_name          = "fee calculator - external - api key"
  content_format        = "openapi-link"
  content_value         = "https://raw.githubusercontent.com/pagopa/mil-apis/main/openapi-mono/fee.yaml"
  product_ids           = [module.mil_product__external__api_key.product_id]
  subscription_required = true
}

resource "azurerm_api_management_api_diagnostic" "fee_calculator_api__external__api_key" {
  identifier               = "applicationinsights"
  resource_group_name      = module.apim.resource_group_name
  api_management_name      = module.apim.name
  api_name                 = module.fee_calculator_api__external__api_key.name
